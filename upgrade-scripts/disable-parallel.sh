...
rm -rf  disabled-modules

curl -s --location --request GET "$dest_url/_/proxy/tenants/$j/modules" \
--header "x-okapi-token: $okapitoken" \
--header 'Cookie: PORTALSESSION=okapi-stg' | cut -d : -f2 | tr -d '"' | grep -  > disabled-modules

sed -i 's/ //g' disabled-modules




rm -rf disabled-modules.json
echo "[" > disabled-modules.json
for i in `cat disabled-modules`; do data=" {\"id\": \"$i\", \"action\": \"disable\"}," ; echo $data >> disabled-modules.json; done 
sed '$ s/,$//g' -i  disabled-modules.json
echo "]" >> disabled-modules.json
...
curl -s --location --request POST "$dest_url/_/proxy/tenants/$j/install?deploy=false&tenantParameters=loadSample%3Dfalse%2CloadReference%3Dtrue" --header "x-okapi-token: $okapitoken" --header 'Content-Type: application/json' --data-binary @disabled-modules.json --max-time 1;
...
tenant_ids=$(cat tenants)

pending_tenants=()
for j in $tenant_ids; do
    response=$(curl -s --location --request GET "$dest_url/_/proxy/tenants/$j/install" --header "x-okapi-token: $okapitoken" --header 'Content-Type: application/json')
    if [[ $response =~ "invoke" ]] || [[ $response =~ "pending" ]]; then
        pending_tenants+=("$j")
    fi
done

# Print pending tenants
if [[ ${#pending_tenants[@]} -gt 0 ]]; then
    echo "Tenants not finished yet:"
    for tenant in "${pending_tenants[@]}"; do
        echo "$tenant"
    done
else
    echo "All tenants have completed installation."
fi
