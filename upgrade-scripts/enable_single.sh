...
curl -s --location --request DELETE "$dest_url/_/proxy/tenants/uoj/install" --header "x-okapi-token: $okapitoken" --header 'Content-Type: application/json'


curl  -s --location --request POST "$dest_url/_/proxy/tenants/uoj/install?deploy=false&tenantParameters=loadSample%3Dfalse%2CloadReference%3Dtrue" --header "x-okapi-token: $okapitoken" --header 'Content-Type: application/json'  --data-binary @modules-enable.json ; 

####### check tenant status before enable next tenant #################


while true; do
    output=$(curl -s --location --request GET "$dest_url/_/proxy/tenants/uoj/install" --header "x-okapi-token: $okapitoken" --header 'Content-Type: application/json')

    if echo "$output" | grep -q -e "invoke" -e "pending"; then
        echo "Installation not completed yet For Tenant uoj. Retrying..."
        sleep 10
    else
      echo "Installation completed successfully For Tenant uoj !"
      break
   fi
